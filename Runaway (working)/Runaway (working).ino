...
void loop()
{
  ...
  if(cur_time - prev_time > UPDATE_TIME_MS && start_flag)
  {
    ...
    float targetVel = angleError*Rp + Ri*angleErrorSum;

    // Cruise (wheel speed) control
    float errL = targetVel - vL;
    float errR = targetVel - vR;
    errTotL += errL*delta_t;
    errTotR += errR*delta_t;
    PWM_left = errL*Kp + Ki*errTotL;
    PWM_right = errR*Kp + Ki*errTotR;
