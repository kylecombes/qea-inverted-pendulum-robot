...
//    short targetSpeed = (millis() < startTime + RACE_TIME) ? DESIRED_VEL : 0;
    float vLError = vL - DESIRED_VEL;
    float vRError = vR - DESIRED_VEL;
    errAccumL += vLError*delta_t;
    errAccumR += vRError*delta_t;
    
    PWM_left = targetPWM + (vLError*Kp + Ki*errAccumL);
    PWM_right = targetPWM + (vRError*Kp + Ki*errAccumR);

    //float ave_PWM = (PWM_left + PWM_right) / 2.0;
    //PWM_left = ave_PWM;
    //PWM_right = ave_PWM;
